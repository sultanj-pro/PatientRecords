...
jest.mock('mongoose', () => {
  ...
  class MockModel {
    ...
    static find = jest.fn().mockReturnValue({ sort: jest.fn().mockResolvedValue([mockDoc]) });
  }
...
describe('ai-orchestrator approvalStore', () => {
  ...
  describe('setStatus', () => {
    ...
    it('throws when trying to approve an already-approved recommendation', async () => {
      ...
      Model.findById.mockResolvedValueOnce({ ...jest.requireActual('./approvalStore'), status: 'approved' });
      // Status transitions are enforced — re-approving should be rejected
      await expect(setStatus('mock-id-123', 'approved')).rejects.toBeDefined();
    });
  });
});
